...
disamb_party <- function(x, party_ref, country, year = NULL, origin = NULL) {
  ...
  cna <- is.na(country)
  if (cna && (length(cna) == 1)) {
    stop("country must be a string identifying the country of the data or a
         vector of strings identifying the country of each observation in
         the data.")
  }
  ...
  if ((length(yna) == 1) && yna) {
    stop("year not provided correctly. If a four-digit numeric scalar, year
         will be applied to all observations. If a numeric vector, year will
         be applied to each observation. If a character object, year must refer
         to a variable in the data. If NULL, year is assumed to be the current
         year.")
  }
  ...
  if (!is.null(origin)) { # CASE: 'origin' *not* NULL
    ...
    if (!(origin %in% c("cctld", "country.name",
                        ...
                        "wvs", "country.name.en.regex",
                        "country.name.de.regex"))) {
      stop("origin must be a valid origin for countrycode function. See
           help(countrycode). If origin is not specified, this function will
           try to resolve a valid input for 'origin'.")
  ...
  if (is.null(origin)) {
    origin_hits <- purrr::map_lgl(
      countrycode::codelist, function(xx) all(unique(country) %in% xx)
    )
    ...
    origin_hits <- origin_hits[origin_hits %in% c(
      "cctld", "country.name",
      "country.name.de", "cowc", "cown", "dhs", "ecb",
      "eurostat", "fao", "fips", "gaul", "genc2c", "genc3c",
      "genc3n", "gwc", "gwn", "imf", "ioc", "iso2c", "iso3c",
    ...
    if (length(origin_hits) > 0) {
      ...
      cat(
        paste0(
          "\nCountrycode origin format not provided. Assuming: ", "\"",
          clean_origin,
          "\" (See countrycode package)\n"
        )
      )
      ...
      origin_hits <- purrr::map_int(
        countrycode::codelist, function(xx) sum(unique(country) %in% xx)
      )

      # if these are English country names
      if ("country.name.en" %in% names(
        origin_hits[origin_hits == max(origin_hits)])
      ) {
        # # change Northern Ireland to UK
        # fixed_country <- ifelse(country %>% str_detect(
        #regex("Northern Ireland", ignore_case = TRUE)),
        #                         "United Kingdom",
        #                         country)

        # separate resolved + unresolved
        resolved_cn <- country[(country %in%
                                  countrycode::codelist$country.name.en)] %>%
          unique()
        toresolve_cn <- country[!(country %in%
                                    countrycode::codelist$country.name.en)] %>%
          unique()


        # if no easy hits, move to English regex (consider adding
        # fallbacks/options to this?)
        ctry_mtchs <- countrycode::codelist$country.name.en.regex %>%
          purrr::map(
            function(xx) {
              stringr::str_which(string = toresolve_cn,
                                 pattern = stringr::regex(xx,
                                                          ignore_case = TRUE))
            }
          )
            ...
            tibble::tibble(
              ...
              dplyr::add_row(
                cc_row = purrr::map_int(
                  resolved_cn,
                  function(xx) {
                    which(countrycode::codelist$country.name.en %in% xx)
                  }
                ),
        ...
        stop("Cannot resolve origin format of country name or identifier.
             Try explicitly specifying origin format & checking country
             identifiers for errors/misspellings.")
        ...
        best_origin_mtch <- names(
          origin_hits[origin_hits == max(origin_hits)][1]
        )
        ...
        if (sum(is.na(x$jnry_country)) == 0) {
          # NB: warn the user that this is the case, and that the data
          # will be incomplete
          cat("\nDifficulty matching country names. All country identifies
              successfully matched, but recommend checking integrity of
              matches.")
        } else {
          # NB: warn the user that this is the case, and that the data
          # will be incomplete
          cat(
            paste0(
              ...
              sum(is.na(x$jnry_country)),
              " observations had no match in the countrycode codelist\nand
              therefore have no assigned country name.\nRecommend checking
              country identifiers for errors/misspellings and trying again.\n"
            )
          )
        }
      }
  ...
  if (is.null(year)) { # CASE: NULL
    ...
    # if year is provided, but it is not the correct class or
    # is empty --> BREAK
    stop("year not provided correctly. If a four-digit numeric scalar,
         year will be applied to all observations. If a numeric vector,
         year will be applied to each observation. If a character object,
         year must refer to a variable in the data. If NULL, year is assumed
    ...
    if (!all(stringr::str_length(year) == 4)) {
      stop("year not provided correctly. If a four-digit numeric scalar,
           year will be applied to all observations. If a numeric vector,
           year will be applied to each observation. If a character object,
           year must refer to a variable in the data. If NULL, year is assumed
    ...
    if (!(year %in% names(x))) {
      stop("year not provided correctly -- if provided as a string, must refer
    ...
    if (!is.numeric(year) || !all(stringr::str_length(year) == 4)) {
      stop("year not provided correctly -- must be numeric and consist of
           only four-digit numbers")
    }

    # create a new column for jnry_year -- wasteful, but...
    # efficient and doesn't rename user variables
    x <- x %>% dplyr::mutate(jnry_year = year)
    ...
    stop("year not provided correctly. If a four-digit numeric scalar,
         year will be applied to all observations in the input data.
         If a numeric vector, year will be mapped to observations in the data.
         If a character object, year must refer to one variable in the data.
  ...
  if (any(party_hits)) {
    ...
    names(cons_refs) <- names(cons_refs) %>% stringr::str_replace("opn",
                                                                  party_ref)
    ...
    if (exists("quar_x")) {
      x <- x %>% dplyr::bind_rows(quar_x)
      cat("\nNote that one or more NAs were present in party, country, or
          year variables.")
    }
    ...
    if (sum(pf_name_hits) > 0) {
      ...
      cat(stringr::str_c(
        "\n-----------------------------------------------------------\nFound ",
        sum(cons_refs$match_exact),
        " exact matches of ",
        nrow(cons_refs),
        "\nparty x year observations.
        \n-----------------------------------------------------------\n"))
      cat("\nParty identifier (party_ref) had some party names without exact
          match in Party Facts data.\n\n---Trying heuristic matching
          (English only)---\n")
    } else {
      # note in 'cons_refs' there are 0 exact matches
      cons_refs <- cons_refs %>% dplyr::mutate(match_exact = FALSE)

      # warn user about heuristic matching
      cat("\nParty identifier (party_ref) had NO exact matches in Party Facts
    ...
    if (nrow(col_heur_hits) > 0) { # CASE: Yes
      ...
      if (any((purrr::map_int(heur_pfcp_refs, length)) > 2)) {
        ...
        cat("\nSome parties have multiple heuristic matches. These have been
            ignored.\nReview party identifiers for, e.g., cases of joint lists
            or other\nambiguities that would prevent identifying single
            parties.\nNOTE: This may cause bad fuzzy matches. Review those
            carefully.\n")

        # remove unambiguous matches
        col_heur_hits <- col_heur_hits %>%
          dplyr::filter(purrr::map_int(heur_pfcp_refs, length) < 3)
        heur_pfcp_refs <- heur_pfcp_refs[purrr::map_int(heur_pfcp_refs,
                                                        length) < 3]
      }
      #############################################################
      ...
      for (i in seq_along(heur_pfcp_refs)) {
      ...
      if (length(cr_name_fields) == 1) {
        ...
        # rebuild 'cons_refs' by stacking 'keep' with 'to_match'
        cons_refs <- keep %>% dplyr::bind_rows(to_match)
        ...
        for (i in seq_along(cr_name_fields)) {
          ...
          # merge subset of 'to_match' to PF, using name field 'i'
          party_to_match[[i]] <- to_match %>%
            dplyr::filter(
        ...
        # reconstitute 'to_match' frame
        to_match <- dplyr::bind_rows(party_to_match)
    ...
    if (all(cons_refs$match_exact | cons_refs$match_heur)) {
      # CASE: YES, COMPLETE AFTER HEURISTIC MATCHING
      ...
      pf_names <- names(cons_refs)[stringr::str_detect(names(cons_refs),
                                                       "^name")]
      ...
      cat(paste(
        ...
        "\nparty x year observations.
        \n-----------------------------------------------------------\n"))
      cat("\nParty identifier (party_ref) had some party names without
      ...
      # get their heuristic variations [is this step necessary? Seems prudent,
      # but invites disagreements]
      ...
      for (ii in seq_along(pfcp_name_fields)) {
        fuzz_matches[[ii]] <- purrr::map(
          fuzz_par_target,
          function(xx) {
            pfhs <- sapply(
              xx,
              function(xxx) {
                agrep(xxx,
                      unique(pfcp[[pfcp_name_fields[ii]]]),
                      max.distance = 0.1)
              }
            )
            if (length(unlist(pfhs)) == 0) {
              return(NULL)
            } else {
      ...
      fuzz_success <- fuzz_matches %>%
        purrr::map_int(function(xx) sum(!purrr::map_lgl(xx, is.null)))
      ...
      #############################################################
      ## Loop again over parties left to fuzzy match --
      ## put party names into cons_refs
      for (ii in seq_along(fuzz_ct_matches)) {
        # CASE: ONE POSITIVE FUZZY MATCH
        if (fuzz_ct_matches[ii] == 1) {
          cons_refs[[fuzz_best]] <- ifelse(
            cons_refs$opn %in% names(fuzz_ct_matches)[ii],
          ...
          for (jj in seq_along(fuzz_to_check)) {
            # get distances
            fuzz_adist[jj] <- utils::adist(x = names(fuzz_ct_matches)[[ii]],
                                           y = fuzz_to_check[jj])
          }

          # if the answers are equally distant...
          # just take the first (fix later?)
      ...
      if (!"wikipedia" %in% names(cons_refs)) {
        cons_refs[["wikipedia"]] <- NA
      }
      if (!"partyfacts_id" %in% names(cons_refs)) {
        cons_refs[["partyfacts_id"]] <- NA
      }
      ...
      if (length(pf_names) == 1) {
        ...
        cons_refs[["partyfacts_name"]] <- NA
        for (ii in seq_along(pf_names)) {
          cons_refs[["partyfacts_name"]] <- ifelse(
            !is.na(cons_refs[[pf_names[ii]]]),
            cons_refs[[pf_names[ii]]],
            cons_refs[["partyfacts_name"]]
          )
        }
      ...
      if (exists("quar_x")) {
        x <- x %>% dplyr::bind_rows(quar_x)
        cat("\nNote that one or more NAs were present in party, country,
            or year variables.")
      }
